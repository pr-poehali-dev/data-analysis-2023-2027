  ...
  return (
    <section id={id} className="relative h-screen w-full snap-start flex flex-col justify-center p-8 md:p-16 lg:p-24">
      <div className={`flex flex-col ${image ? 'lg:flex-row lg:items-center lg:gap-16' : ''}`}>
        ...
        {image && (
          <motion.div
            className="mt-8 lg:mt-0 flex-shrink-0 lg:w-[380px] xl:w-[440px]"
          ...
          >
            <img
              src={image}
              alt={title}
              className="w-full aspect-square object-cover rounded-2xl border border-amber-900/40 shadow-2xl shadow-amber-900/20"
            />
          </motion.div>
        )}
      </div>
